...
double x = Math.Pow((x2-x1), 2); // возведение в квадрат
double y = Math.Pow((y2-y1), 2);
double z = Math.Pow((z2-z1), 2);

double result = Math.Sqrt(x + y + z); // Math.Sqrt - возведение в квадрат
System.Console.WriteLine(Math.Round (result, 2)); // Math.Round - округляет количество знаков после запятой, 3 - то кол-во после запятой